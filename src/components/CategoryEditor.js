...
function CategoryEditor({ category, items, onSave }) {
  ...
  return (
    <div className="mb-8 p-1 bg-white rounded-lg shadow">
      <Accordion activeIndex={activeIndexes} className="w-full py-4 font-bold text-lg" onTabChange={onTabChange}>
        <AccordionTab contentClassName='font-light py-0 text-sm' onClick={() => WebApp.HapticFeedback.impactOccurred('soft')} headerClassName='accord' key={items.id} header={category}>
